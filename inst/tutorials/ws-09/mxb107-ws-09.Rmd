---
title: "MXB107 Worksheet 9 - Small Sample Inference"
output:
  learnr::tutorial:
    theme: yeti
    includes:
...
House paint is sold in containers labelling the coverage and drying time for a single coat of paint.  
One manufacturer's packaging claims that a four-litre can of paint covers 37 square metres.  
This claim is tested by drawing ten cans of paint at random and painting ten identical surfaces using the cans of paint and the same methods.  
The resulting coverage for each can of paint:
<br>
...
B <- c(10.2,9.4,11.8,9.1,8.3)
(A-B)%>%
  kable(col.names = "Difference Between Tyre A and Tyre B", align = "c","pipe")
```  
:::
:::
...
data.frame(x1,x2)%>%
  kable(col.names = c("Technique 1", "Technique 2"), align = "c","pipe")
...
:::{.table-narrow}
```{r, echo = FALSE}
df <- tibble('Dye A' = c(10,12,9,8,15,12,9,10,15),
             'Dye B' = c(8,11,10,6,12,13,9,8,13))
kable(df)%>% column_spec(column = 1:2,width = "2cm")
```
:::
...
data.frame(x1,x2)%>%
  kable(col.names = c("Store Brand", "Name Brand"), align = "c")
...
data.frame(x1,x2)%>%
  kable(col.names = c("Before","After"),align = "c")
